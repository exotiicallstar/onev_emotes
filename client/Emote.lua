...
function OnEmotePlay(EmoteName, name, textureVariation)
    ...
    if animOption then
        ...
        if animOption.PtfxAsset then
            ...
            TriggerServerEvent("onev_emotes:ptfx:sync", PtfxAsset, PtfxName, vector3(Ptfx1, Ptfx2, Ptfx3), vector3(Ptfx4, Ptfx5, Ptfx6), PtfxBone, PtfxScale, PtfxColor)
        else
    ...
    if animOption and animOption.Prop then
        ...
        if animOption.PtfxAsset and not PtfxNoProp then
            TriggerServerEvent("onev_emotes:ptfx:syncProp", ObjToNet(prop))
        end
    end
end
