...
local lastEmoteUse = {}  -- Tabelle zum Speichern des letzten Verwendungszeitpunkts eines Emotes für jeden Spieler
...
RegisterNetEvent("ServerValidEmote", function(target, requestedemote, otheremote)
    ...
    if lastEmoteUse[source] and currentTime - lastEmoteUse[source] < 5 then  -- 5 Sekunden Cooldown
        TriggerClientEvent('showNotification', source, 'Du musst warten, bevor du dieses Emote erneut nutzen kannst.')
        return
    end

    lastEmoteUse[source] = currentTime  -- Aktualisieren des letzten Verwendungszeitpunkts
...
--#region ptfx
RegisterNetEvent("onev_emotes:ptfx:sync", function(asset, name, offset, rot, bone, scale, color)
    if type(asset) ~= "string" or type(name) ~= "string" or type(offset) ~= "vector3" or type(rot) ~= "vector3" then
        print("[onev_emotes] ptfx:sync: invalid arguments for source:", source)
        return
    end
...
RegisterNetEvent("onev_emotes:ptfx:syncProp", function(propNet)
    local srcPlayerState = Player(source).state
    if propNet then
